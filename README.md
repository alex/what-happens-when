...
1. The browser begins fetching external resources linked to the page (CSS, images, JavaScript
files, etc.):
  * The browser initiates separate network requests for each external resource referenced in the HTML document.These requests are sent to the respective servers hosting the resource..
  * The browser waits for the responses from the servers to retrieve the requested resources.

2. At this stage, the browser marks the document as interactive and starts parsing scripts that are in "deferred" mode: those that should be executed after the document is parsed. This includes:
  * Scripts with the defer attribute: These scripts are deferred and will be executed after the
    HTML parsing is complete, but before the DOMContentLoaded event is fired.
  * Scripts loaded dynamically or added programmatically: If any scripts are added to the document dynamically or through JavaScript, they are parsed and executed in the order they are added.

3.  The document state is set to "complete" and a "load" event is fired:
  * Once all external resources have been fetched and parsed, and all deferred scripts have been executed, the browser updates the document state to "complete."
  * The browser then triggers a "load" event on the window object, indicating that the page and
    all its resources have finished loading.
  * This event can be captured and handled by JavaScript code to perform additional actions or manipulate the page content.

Note: There is never an "Invalid Syntax" error on an HTML page. Browsers are designed to handle various degrees of invalid HTML and attempt to fix any syntax errors encountered during the
parsing process. Browsers employ error correction mechanisms to interpret and render the content as intended, even in the presence of minor syntax issues. This ensures that web pages with
minor markup errors can still be displayed and function properly.
