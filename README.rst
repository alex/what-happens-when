...
The goal of the DNS lookup is to translate the HTTP hostname (human-friendly) into an IP address that will be used by the network stack.
...
* If no cache information for this hostname is found, the DNS server will use recursion to fully resolve the name. 
   ...
   * For the example hostname "drive.google.com", it will ask the top-level ".com" DNS server which will forward to the "google.com" DNS server and so on, until the authoritative DNS server is found.
