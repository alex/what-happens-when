...
* If not found, the browser calls ``gethostbyname`` library function (varies by
  OS) to do the lookup.
* In a recursive DNS lookup, when a server receives a request for an uncached domain,
 it queries authoritative DNS servers hierarchically—root,
 TLD, and domain-specific servers. This process continues until the server
 obtains the IP address, ensuring accurate and efficient resolution.
