...
--------------------------
The HTTPD (HTTP Daemon) server is responsible for handling requests and responses on the server-side. The most common HTTPD servers for Linux are Apache and Nginx, while Windows commonly uses IIS.

* The HTTPD (HTTP Daemon) receives the request.
* The server breaks down the request to the following parameters:
   * HTTP Request Method (either ``GET``, ``HEAD``, ``POST``, ``PUT``,
     ``PATCH``, ``DELETE``, ``CONNECT``, ``OPTIONS``, or ``TRACE``). When a URL is entered directly into the address bar, the method is typically ``GET``.
   * The domain, in this case, is "google.com."
   * The requested path/page, in this case, is "/" (as no specific path/page was requested, "/" is the default).
* The server verifies that there is a Virtual Host configured on the server
  that corresponds to google.com.
* The server verifies and confirms that "google.com" can accept GET requests.
* The server also checks whether the client is allowed to use this method 
  (based on IP authentication, etc.).
* If the server has a rewrite module installed (e.g., mod_rewrite for Apache or URL Rewrite for
  IIS), it attempts to match the request against the configured rules. When a matching rule is found, the server uses that rule to rewrite the request.
* The server retrieves the content that corresponds to the request. In our case, it defaults to
  the index file since "/" is the main file (although some cases may override this, it's the most common approach).
* The server processes the file according to the handler. If Google is running on PHP, the
  server uses PHP to interpret the index file and streams the output to the client.
