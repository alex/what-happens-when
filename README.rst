...
First of all, after you input a web address in the search bar the browser initiates a request to access that specific webpage, by translating the input human-readable domain name ("google.com") into an IP address understandable by computers, it sends a DNS request to a DNS resolver, which might be within your ISP or third-party DNS servers like Google's 8.8.8.8, searches for the corresponding IP adress associated with "google.com". Once the IP address is found, it's returned to your computer, enabling it to establish a connection with Google's server by using the TCP/IP protocol suite. TCP is responsible for breaking data into packets, sequencing them, error-checking, and ensuring reliable delivery. It manages the establishment and termination of connections. On the other hand, IP handles addressing and routing, enabling packets to traverse the internet and reach the correct destination, but this without protection is risky, as unauthorized access and potential threats could occur, so this where a Firewall comes in handy, typically installed on your local network, ISP, or server, examines incoming and outgoing traffic, by comparing the traffic against predefined rules to allow or block specific packets based on security policies. But a Firewall alone isn't enough, so the browser uses another type of protection to ensure secure connection, called "HTTPS", for secure websites like Google (identified by "https"), the browser and server establish an encrypted connection using a set of protocols, "SSL" (Secure Socket Layer) and "TLS" (Transport Layer Security), so that the data transmitted between your browser and Google's server is secured, preventing unauthorized access or tampering. But what happens if many type "google.com" at the same time? a simple server would basically faild with that many incoming traffic, here is where the Load-Balancer comes in handy, it disributes traffic among multiple servers, so that it ensures optimal performance, prevents overloading, and enhances the site's reponsiveness. After the request reaches the Load-Balancer and gets distributed to a server which hosts the website, the web server processes the request, retrieves the requested resources (HTML, CSS, Javascript files), and prepares them for transmission back to the browser. That is only for simple websites which require work done on the local computer rather than the hosting server, but in the case of sophisticated web applications (e.g. Gmail), the web server interacts with an application server, which processes dynamic content, executes code, performs tasks integral to the application's functionality and interacts with databases (for applications requiring data storage or retrieval), the database server stores, manages, and retrieves data based on the application's needs, ensuring data integrity and accessibility.
