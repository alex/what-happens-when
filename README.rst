...
* If the server has a rewrite module installed (like mod_rewrite for Apache or
  ...
  rewrite the request.
* If the server is configured with a load balancer, the request is forwarded according to the configured balance rule. The 
  reverse proxy load balancing is the most common in the request of the proxy server to 
  forward  the request to the upstream and downstream back-end server for reducing server pressure.
