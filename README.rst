...
* Browser checks if the domain is in its cache. The brower maintains a repository
   of DNS records for a fixed duration of time for websites you have previously
   visited. (to see the DNS Cache in Chrome, go to
   `chrome://net-internals/#dns <chrome://net-internals/#dns>`_).
* If not found in the browser's cache, the browser will make a system call
   (i.e., gethostname on Windows). ``gethostbyname`` checks if the hostname can be
...
* If the DNS server is on the same subnet (a segmented piece of a larger network),
   the network library follows the ``ARP process`` below for the DNS server.
