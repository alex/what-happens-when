...
and the OS interrupts. When you press the key "g" or any other key on the keyboard depending on what you are browsing, the browser receives the
...
* If the website is in the list, the browser sends its request via HTTPS
  instead of HTTP and a padlock icon would be seen on the address bar. Otherwise, the initial request is sent via HTTP.
...
* If ``gethostbyname`` does not have it cached nor can find it in the ``hosts``
  file then it makes a request to the DNS (Domain Name Systems) server configured in the network
