...
* If not found, the browser calls ``getaddressinfo`` library function (varies by
  OS) to do the lookup.
* ``getaddressinfo`` checks if the hostname can be resolved by reference in the
  local ``hosts`` file (whose location `varies by OS`_) before trying to
  resolve the hostname through DNS.
* If ``getaddressinfo`` does not have it cached nor can find it in the ``hosts``
...
JavaScript files, static files, etc.).
