...
----------------------------------------------------
1. DNS Request: Unveiling the Mystery
Since the advent of HTTP between 1989-1991 by its predecessor, it has progress from being just one protocol where individuals can send a textual based document from one computer to another to being a concept on which most businesses are built upon to reach their targeted audience via worldwide web powered by the internet. However, it is really good to understand how the processes involved in the operations of the internet we now see today unfold one step after the other in the subsequent paragraphs.
To start with, let’s discuss how a website/web page is requested and given back to the user in the user interface. When a DNS (Domain Name System) which we commonly identify as the website address or the URL (Uniform Resource Locator) which may be a full or partial path to a web page you want to visit using https://www.google.com as a case study, the address you actually requested is not the string of text you type in the URL; It is in the real sense of it a string of numbers that probably looks like this 192.2.40.80 called IP that is labelled with the textual string google.com.
Human being unlike the computers, do not understand or easily remember this IP address, hence the introduction of Domain Name System was needful. This concept helps to tie an IP to the string https://www.google.com for easy reference. But this process that happens really fast provided your connection has good speed, involves several chain of processes to complete that single task. But how does it work under the hood? Whenever you type the address of a web page, the address is matched to an IP address. This matching itself will have to be a chain of process which starts as a recursive search for the match first of all in the cache or storage of the computer (OS) which sends the request. If there’s no match in the client’s OS, it reaches out to the resolver server, then to the root if not also found with the resolver server which your ISP. At this point, any of the points that has the IP returns it back to the client and the IP is used to start connection. If not the root is searched and if a match is found it returns the IP of the Domain Name. IP is the only way computers recognize and communicate with one another. Once this IP corresponding to the Domain Name is found at the root, then communication can be established between the two systems involved and information/data can be transmitted between them also. However, there are times that the process may not be as simple as it was first illustrated, when the domain name’s IP address cannot readily be found on the requesting client side OS probably because the address is not stored on the client computer. This is when it is imperative for the root server to reach out to its contemporary systems involved in domain name system (DNS).
Smartly enough, the root server knows that when a domain name is not resident in it, it must be with the DNS server which is the major custodian of the whole world domain name register. And the DNS server host most TLD (Top Level Domain) such as .com, .org, .net etc which is where all domain names themselves are hosted. Once search has reached here and the IP is returned to the client side, connections are made between IP of the client and the server that should serve the web page google.com.

2. The Role of TCP/IP
As mentioned before, IP is an important part of the TCP/IP model which by itself is a standard, which also surface as the Layer 4 and Layer 7 of the OSI Model. And we will see its importance shortly. By nature HTTP (Hyper Text Transport protocol) is a stateless protocol because, its main purpose from inception was just to send textual based data via the internet and there was no considerations for perfection and checks as to whether data are properly and correctly delivered at each end. Nowadays, data are sent in packets bit by bit, to and from different computers. With the birth TCP/IP model, packets that are sent are monitored and the TCP/IP model enables this, by acknowledging what each side (client side and server side) sends per time and ensuring that if there are discrepancies in the packets received from each end, they are resent correctly and completely. There is hardly no manufacturer of computer hardware, that does not regard the TCP/IP model in that, computer systems can only communicate effectively using this model/framework and there must be uniformity of purpose so that, computer systems know how to effectively connect and interacts with one another with ease and no confusion.

3. HTTPS/SSL: Securing the Web
Another point to mention here is that, website which is built on the concept of HTTP protocol, now handles more sophisticated communication that must be securely served, otherwise things would go wrong and users will not have trust in the framework. That way, there is need for securing every page of a website so that activities that are performed there are reliably safe. HTTPS, the ‘S’ for secure, allows the HTTP version which was unsecured by mean of adding another layer of protection called SSL (Secure Socket Layer) to protect users’ experience on the website, so that information shared on the visited website are channeled through a secured pipeline without fear of data breaches. By adding SSL certificate to a website, we readily see a padlock icon on that page by the URL, signifying that it is a secure website and this by itself boost users’ confidence to visit that website.

4. Load Balancers: Managing Traffic To Web Servers, Application Servers etc
After your website is secured, users’ trust in your website, more users/traffic tend to hit your website. It is however, important to have a Load Balancer in place to distribute the traffic to the various web pages/resources they want to fetch using different distribution algorithms such as round robin, least-connection-first-scheduling. Usually a Load Balancer interfaces directly with the web server. The web server major purpose in life is to serve static pages to the users. Any request other than static web pages and more sophisticated, are then transferred to the application servers through a servelet capable of handling business logic for the website. This business logic is simply dependent on how the owner of the website wants a particular query to a particular web page to be presented to users for better users’ experience. Sometimes, if the resources could not be readily fetched from the application server, it could be read from the database by the application server, process it and gives it back to the web server.
