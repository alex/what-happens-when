...
When you press the Enter key on your keyboard, your computer sends a signal to a
program that translates it into a code. This code is called VK_RETURN and it means
that you want to confirm or execute something. The program then passes the code to
anotherprogram that checks which window on your screen is active. In this case, it
is the browser’s address box, where you type the website you want to visit. The second
program then sends a message to the window, telling it that you have pressed VK_RETURN.
The window has a function that knows what to do when it receives this message. In this
case, it tells the browser to go to the website you have typed in the address box.
